...
After the machine restarts, run the script again to install Java. Builds below AAV05 require Sun Java 1.6. Builds above and including AAV05 require OpenJDK 7. To install either of them, select the 2nd option in the main menu and select option 2 (or 3, if the build demands JDK 7). The script can also change the default Java version, which can be useful if compiling different builds.
...
To initialize a repository tree using one of the manifests provided by this project, execute a command like this (see the list of manifests above for available `<build>`s):
    ...
    repo init -u https://github.com/froyocomb/android.git -b thou-shalt-take-the-L -m <build>.xml --depth=1
